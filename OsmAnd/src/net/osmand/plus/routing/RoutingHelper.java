...
public class RoutingHelper {
	...
	public List<Location> getCurrentRoute() {
		return currentGPXRoute == null || currentGPXRoute.points.isEmpty() ? route
				.getImmutableLocations() : Collections
	...
	public Location setCurrentLocation(Location currentLocation) {
		...
		synchronized (this) {
			...
			if (route.isEmpty()) {
				...
				// 1. Update current route position status according to latest
				// received location
				...
				if (currentRoute > 0) {
					double dist = getOrthogonalDistance(currentLocation,
				...
				boolean wrongMovementDirection = checkWrongMovementDirection(
						currentLocation, next);
				if (wrongMovementDirection
						&& currentLocation.distanceTo(routeNodes
								.get(currentRoute)) > 2 * POSITION_TOLERANCE) {
				...
				if (currentRoute > 0) {
					double dist = getOrthogonalDistance(currentLocation,
							routeNodes.get(currentRoute - 1),
							routeNodes.get(currentRoute));
					double projectDist = mode == ApplicationMode.CAR ? POSITION_TOLERANCE
							: POSITION_TOLERANCE / 2;
					locationProjection = new Location(locationProjection);
					if (dist < projectDist) {
						Location nextLocation = routeNodes.get(currentRoute);
						LatLon project = getProject(currentLocation,
								routeNodes.get(currentRoute - 1),
								routeNodes.get(currentRoute));
						...
						if (locationProjection.hasBearing()) {
							float bearingTo = locationProjection
		...
		if (calculateRoute) {
			recalculateRouteInBackground(lastFixedLocation, finalLocation,
					currentGPXRoute, route.isCalculated() ? route : null);
		}
		return locationProjection;
	}

	private double getOrthogonalDistance(Location loc, Location from,
			Location to) {
	...
	private LatLon getProject(Location loc, Location from, Location to) {
		return MapUtils.getProjection(loc.getLatitude(), loc.getLongitude(),
				from.getLatitude(), from.getLongitude(), to.getLatitude(),
				to.getLongitude());
	}

	private int lookAheadFindMinOrthogonalDistance(Location currentLocation,
			List<Location> routeNodes, int currentRoute, int iterations) {
		double newDist;
		double dist = Double.POSITIVE_INFINITY;
		int index = currentRoute;
		while (iterations > 0 && currentRoute + 1 < routeNodes.size()) {
			newDist = getOrthogonalDistance(currentLocation,
					routeNodes.get(currentRoute),
					routeNodes.get(currentRoute + 1));
			if (newDist < dist) {
				index = currentRoute;
	...
	private boolean updateCurrentRouteStatus(Location currentLocation) {
		List<Location> routeNodes = route.getImmutableLocations();
		int currentRoute = route.currentRoute;
		// 1. Try to proceed to next point using orthogonal distance (finding
		// minimum orthogonal dist)
		while (currentRoute + 1 < routeNodes.size()) {
			double dist = currentLocation.distanceTo(routeNodes
					.get(currentRoute));
			if (currentRoute > 0) {
				dist = getOrthogonalDistance(currentLocation,
						routeNodes.get(currentRoute - 1),
						routeNodes.get(currentRoute));
			}
			...
			int newCurrentRoute = lookAheadFindMinOrthogonalDistance(
					currentLocation, routeNodes, currentRoute,
					longDistance ? 15 : 8);
			double newDist = getOrthogonalDistance(currentLocation,
			...
			if (longDistance) {
				...
				if (dist > POSITION_TOLERANCE) {
					...
					// case if you are getting close to the next point after
					// turn
					// but you have not yet turned (could be checked bearing)
					if (currentLocation.hasBearing()
							|| lastFixedLocation != null) {
						float bearingToRoute = currentLocation
								.bearingTo(routeNodes.get(currentRoute));
						float bearingRouteNext = routeNodes
								.get(newCurrentRoute).bearingTo(
										routeNodes.get(newCurrentRoute + 1));
						float bearingMotion = currentLocation.hasBearing() ? currentLocation
								.getBearing() : lastFixedLocation
								.bearingTo(currentLocation);
						double diff = Math.abs(MapUtils.degreesDiff(
								bearingMotion, bearingToRoute));
						double diffToNext = Math.abs(MapUtils.degreesDiff(
								bearingMotion, bearingRouteNext));
						if (diff > diffToNext) {
							if (log.isDebugEnabled()) {
								log.debug("Processed point bearing deltas : "
										+ diff + " " + diffToNext);
							}
							processed = true;
						}
		...
		if (currentRoute > routeNodes.size() - 3
				&& currentLocation.distanceTo(lastPoint) < POSITION_TOLERANCE) {
			...
			if (Math.abs(diff) > 135f) {
				...
				if (d > POSITION_TOLERANCE) {
					if (makeUTwpDetected == 0) {
						...
						// require 5 sec since first detection, to avoid false
	...
	/**
	 * Wrong movement direction is considered when between current location
	 * bearing (determines by 2 last fixed position or provided) and bearing
	 * from currentLocation to next (current) point the difference is more than
	 * 90 degrees
	 */
	public boolean checkWrongMovementDirection(Location currentLocation,
			Location nextRouteLocation) {
		if ((currentLocation.hasBearing() || lastFixedLocation != null)
				&& nextRouteLocation != null) {
			float bearingMotion = currentLocation.hasBearing() ? currentLocation
					.getBearing() : lastFixedLocation
					.bearingTo(currentLocation);
			float bearingToRoute = currentLocation.bearingTo(nextRouteLocation);
			double diff = MapUtils.degreesDiff(bearingMotion, bearingToRoute);
			// 6. Suppress turn prompt if prescribed direction of motion is
			// between 45 and 135 degrees off
	...
	private synchronized void setNewRoute(RouteCalculationResult res,
		...
		route = res;
		if (isFollowingMode) {
			// try remove false route-recalculated prompts by checking direction
			// to second route node
			boolean wrongMovementDirection = false;
			Location prev = res.getNextRouteLocation();
			if (prev != null) {
				int i = 1;
				while (res.getNextRouteLocation(i) != null
						&& prev.distanceTo(start) < POSITION_TOLERANCE) {
				...
				wrongMovementDirection = checkWrongMovementDirection(start,
						prev);
				// set/reset evalWaitInterval only if new route is in forward
				// direction
				if (!wrongMovementDirection) {
					evalWaitInterval = 3000;
				} else {
			...
			// (but see also additional 60sec timer for this message in
			// voiceRouter)
			// If route is in wrong direction after one more setLocation it will
			// be recalculated
	...
	public synchronized NextDirectionInfo getNextRouteDirectionInfo(
		...
		NextDirectionInfo i = route.getNextRouteDirectionInfo(info,
				lastFixedLocation, toSpeak);
		if (i != null) {
			i.imminent = voiceRouter.calculateImminent(i.distanceTo,
					lastFixedLocation);
		}
		return i;
	}
	...
	public synchronized AlarmInfo getMostImportantAlarm(MetricsConstants mc) {
		...
		if (mxspeed != 0 && lastFixedLocation != null
				&& lastFixedLocation.hasSpeed()) {
			float delta = 5f / 3.6f;
	...
	public synchronized String getCurrentName() {
		NextDirectionInfo n = getNextRouteDirectionInfo(
	...
	public synchronized NextDirectionInfo getNextRouteDirectionInfoAfter(
		...
		NextDirectionInfo i = route.getNextRouteDirectionInfoAfter(previous,
				to, toSpeak);
		if (i != null) {
	...
	private class RouteRecalculationThread extends Thread implements
			Interruptable {
		...
		public RouteRecalculationThread(String name, Location start,
				LatLon end, GPXRouteParams gpxRoute,
		...
		public void run() {
			...
			synchronized ((OsmandApplication)context) {
				res = provider.calculateRouteImpl(start, end, mode, service,
						context, gpxRoute, previousRoute, fastRoute, leftSide,
						this);
			}
			if (interrupted) {
			...
			if (res.isCalculated()) {
				...
				showMessage(
						context.getString(R.string.error_calculating_route)
				...
				if (res.getErrorMessage() != null) {
					...
					showMessage(
							context.getString(R.string.empty_route_calculated),
							Toast.LENGTH_LONG);
				}
			}
	...
	private void recalculateRouteInBackground(final Location start,
		...
		if (currentRunningJob == null) {
			...
			if (System.currentTimeMillis() - lastTimeEvaluatedRoute > evalWaitInterval) {
				synchronized (this) {
					currentRunningJob = new RouteRecalculationThread(
							"Calculating route", start, end, gpxRoute, previousRoute); //$NON-NLS-1$
